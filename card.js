...
class AqaraFP2Card extends HTMLElement {
  ...
  drawSensorPosition(data, minX, maxX, minY, maxY, cellSize) {
    ...
    // Label — bias alignment inward when the sensor is at/near a canvas edge
    // so the text doesn't clip. Corner mounts put xPos at or near 0/canvas-
    // width; wall mount centres it.
    this.ctx.font = "bold 10px system-ui, sans-serif";
    this.ctx.fillStyle = "rgba(244,67,54,0.9)";
    this.ctx.textBaseline = "top";
    const canvasW = this.renderParams ? this.renderParams.canvasWidth : (maxX - minX + 1) * cellSize;
    const edgePad = 4;       // minimum distance from canvas edge in px
    let labelX = xPos;
    if (xPos < cellSize) {
      this.ctx.textAlign = "left";
      labelX = edgePad;
    } else if (xPos > canvasW - cellSize) {
      this.ctx.textAlign = "right";
      labelX = canvasW - edgePad;
    } else {
      this.ctx.textAlign = "center";
    }
    this.ctx.fillText("SENSOR", labelX, yPos + size + 22);
  }
